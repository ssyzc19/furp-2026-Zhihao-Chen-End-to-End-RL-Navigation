...
def _build_cfg(seed: int):
    ...
    with open_dict(cfg):
        ...
        # Fix missing agents_order configuration
        if 'agents_order' not in cfg.habitat.simulator or OmegaConf.is_missing(cfg.habitat.simulator, 'agents_order'):
            cfg.habitat.simulator.agents_order = ["main_agent"]
    return cfg
...
def run_episode(env, actor_critic, device, max_steps: int = 500):
    ...
    for step in range(max_steps):
        ...
        step_out = env.step(action.squeeze().item())
        if len(step_out) == 4:
            obs, _, done, step_info = step_out
        else:
            obs, done, step_info = step_out

        # Update info only if step_info is a dict
    ...
    # Get final metrics after episode ends
